...
function usage() {
  console.log("Usage: node scripts/taipei-dry-run.mjs <submission-packet.json>");
  console.log("");
  console.log("Writes taipei-automation-plan.json next to the packet. Does not open or submit to official websites.");
}
...
async function main() {
  ...
  const plan = createTaipeiAutomationPlan(packet);
  const outputPath = join(dirname(packetPath), "taipei-automation-plan.json");
