...
- Browser-created drafts preserve `createdAt` separately from `updatedAt`; imported drafts keep existing `createdAt`
- `scripts/validate-case-draft.mjs` validates generated `draft.json` structure, 1-to-5 attachment count, attachment metadata, and saved review state without reading attachment contents or contacting official sites
- `scripts/review-case-workflow.mjs` summarizes local case-folder artifacts, one recommended next action, and next safe commands
...
- GPS may be missing on individual attachments, so OCR and manual location review remain necessary fallbacks
- OCR output still requires manual confirmation; adopted field suggestions are tracked in `fieldReview`
...
- OCR location text candidates are added to `locationAssistance.candidates` as review-only `ocr_text` candidates
- Frontend renders OCR location candidates with `OCR 文字` source labels for manual review
...
- `scripts/review-case-readiness.mjs` flags missing, future, or locally stale violation times as review warnings without automating legal judgment
...
- The packet includes a privacy-preserving `preSubmitReview` summary for user confirmation before final submit
...
- `scripts/encrypt-reporter-profile.mjs` writes an ignored AES-256-GCM encrypted reporter profile envelope using `REPORTER_PROFILE_PASSPHRASE`
- `scripts/prepare-submission.mjs`, `scripts/review-case-readiness.mjs`, and `scripts/validate-reporter-profile.mjs` can read encrypted reporter profiles when `REPORTER_PROFILE_PASSPHRASE` is set
...
- `scripts/taipei-prototype.mjs` writes `taipei-prototype-run.json` and refuses to contact the official site unless required data is complete, matching ready `case-readiness-report.json` and plan fixture reports are provided, and `--allow-network` is explicitly provided
- `scripts/lib/taipei-automation-plan.mjs` includes a human-required `review_pre_submit_summary` stop before final submit
...
- `scripts/run-plan-fixture.mjs` follows the guarded automation plan against a local official-like fixture and stops at the first human gate
- `scripts/review-case-workflow.mjs` treats the plan fixture report as a required safety artifact before guarded official-site handoff
- `scripts/taipei-prototype.mjs` includes a guarded handoff summary with the first human stop and post-submission record guidance
...
- The dry-run plan marks Email verification, declarations, pre-submit summary review, and final submit as human-required stop points
...
- Verification confirmed `preSubmitReviewStatus: "manual_required"` and that the pre-submit summary omits fixture identity number and email values
- Verification confirmed encrypted reporter profile round trip and checked that the encrypted envelope does not contain fixture identity number or email plaintext
...
- Verification confirmed `taipeiPlanFixtureStatus: "stopped_at_human_gate"` and `taipeiPlanFixtureStoppedAt: "stop_before_email_verification"` before any case-field filling, upload, human-stop click, official URL contact, or final submit
- Verification confirmed the workflow checklist recommends `fixture:plan` when the plan fixture report is missing
- Verification confirmed `taipeiPrototypeWithoutReadinessStatus: "blocked_by_readiness_report"`, `taipeiPrototypeWithoutPlanFixtureStatus: "blocked_by_plan_fixture_report"`, `taipeiPrototypeWithReadinessStatus: "ready_for_guarded_browser"`, and a handoff first stop of `stop_before_email_verification`
...
- Real reporter profile data must remain local and ignored; only `reporter-profile.example.json` is committed. If encrypted reporter profiles are used, the passphrase must stay outside the repository
...
- `scripts/new-taipei-prototype.mjs` writes `new-taipei-prototype-run.json` and refuses to contact the official site unless required data is complete, matching ready `case-readiness-report.json` and plan fixture reports are provided, and `--allow-network` is explicitly provided
...
- `scripts/lib/new-taipei-automation-plan.mjs` includes a human-required `review_pre_submit_summary` stop before final submit
...
- `scripts/run-plan-fixture.mjs` verifies that the New Taipei guarded plan stops at the disclaimer gate before filling fields
- `scripts/review-case-workflow.mjs` tracks the New Taipei plan fixture report before guarded official-site handoff
- `scripts/new-taipei-prototype.mjs` includes a guarded handoff summary with the first human stop and post-submission record guidance
- The New Taipei packet points to `https://tvrs.ntpd.gov.tw/`
- The plan stops for disclaimer confirmation, CAPTCHA, Email verification, pre-submit summary review, and final submit
- Verification confirmed `newTaipeiDryRunStatus: "blocked_by_missing_data"`, `newTaipeiPrototypeStatus: "blocked_by_missing_data"`, `newTaipeiDryRunManualStops: 5`, and `newTaipeiSelectorFieldCount: 20`
- Verification confirmed `newTaipeiPrototypeWithoutReadinessStatus: "blocked_by_readiness_report"`, `newTaipeiPrototypeWithTaipeiReadinessStatus: "blocked_by_readiness_report"`, `newTaipeiPrototypeWithoutPlanFixtureStatus: "blocked_by_plan_fixture_report"`, `newTaipeiPrototypeWithReadinessStatus: "ready_for_guarded_browser"`, and a handoff first stop of `stop_before_disclaimer`
...
- Verification confirmed `newTaipeiPlanFixtureStatus: "stopped_at_human_gate"` and `newTaipeiPlanFixtureStoppedAt: "stop_before_disclaimer"` before any field filling, upload, human-stop click, official URL contact, or final submit
...
- `scripts/list-cases.mjs` reads local `case-record.json` files and outputs a case history summary with `lookupPasswordStored` instead of the lookup password value
- Verification confirmed `caseRecordStatus: "needs_missing_data"`, `updatedCaseRecordStatus: "submitted_by_user"`, `caseSummaryOfficialCaseNumber: "TP-FIXTURE-0001"`, `caseSummaryOfficialReceiptStatus: "recorded"`, `caseSummaryLookupPasswordStored: true`, `caseCorrectionStatus: "needs_action"`, `caseCorrectionItemCount: 2`, `caseRecordMarkdownVerification: "ok"`, and preserved pre-submit review plus final-submit as human stops
- Verification confirmed workflow recommendation `complete_official_receipt` when a submitted case is missing the official case number or submitted time
- `index.html` / `app.js` import `case-record.json` and `case-history.json` and display jurisdiction, local/submission/automation status, official case number, lookup-password stored status, submitted time, attachment count, missing count, and human stop count
- `scripts/verify-ui-fixtures.mjs` verifies browser import rendering for single case record and case history fixtures, including lookup-password stored status without rendering the password value
...
npm run taipei:prototype -- cases/case-20260616T031513/taipei-automation-plan.json --readiness-report cases/case-20260616T031513/case-readiness-report.json --plan-fixture-report cases/case-20260616T031513/taipei-plan-fixture-report.json --allow-network
npm run fixture:plan -- cases/case-20260616T031513/submission-packet.json cases/case-20260616T031513/taipei-automation-plan.json
...
npm run new-taipei:prototype -- cases/case-20260616T030331/new-taipei-automation-plan.json --readiness-report cases/case-20260616T030331/case-readiness-report.json --plan-fixture-report cases/case-20260616T030331/new-taipei-plan-fixture-report.json --allow-network
...
All listed commands completed successfully by the 2026-07-09 audit, except where older case IDs are retained as prior evidence. The latest end-to-end verifier used `cases/case-20260708T181739`; the latest draft validation returned `caseDraftValidationStatus: "ok"`, `draftCreatedAtPresent: true`, `emptyAttachmentDraftStatus: "invalid"`, `oversizedDraftStatus: "invalid"`, and rejected invalid fixture drafts, including 7 invalid review-state issues; the latest video attachment verification returned `videoAttachmentStatus: "not_required"`, `videoAttachmentAcceptedByOfficial: true`, and `videoOnlyPhotoAnalysisStatus: "skipped"`; the latest workflow checklist verification returned `workflowChecklistVerification: "submitted_by_user"`, `workflowOfficialReceiptStatus: "recorded"`, `workflowPlanFixtureStatus: "present"`, and `workflowNextAction: "workflow_complete"` after local workflow artifacts were written; the latest plate verification returned normalized candidates `3999-YG` and `3999-B` with patterns `four_digits_two_letters` and `four_digits_one_letter_incomplete`; the latest location candidate verification returned `locationCandidates: 2` and `ocrLocationCandidates: ["傳品牛排"]`; the latest pre-submit review verification returned `preSubmitReviewStatus: "manual_required"`, `preSubmitReviewAttachmentCount: 2`, `taipeiDryRunManualStops: 4`, and `newTaipeiDryRunManualStops: 5`, while omitting fixture identity number and email values; the latest plan-aware fixture verification returned `taipeiPlanFixtureStatus: "stopped_at_human_gate"`, `taipeiPlanFixtureStoppedAt: "stop_before_email_verification"`, `newTaipeiPlanFixtureStatus: "stopped_at_human_gate"`, and `newTaipeiPlanFixtureStoppedAt: "stop_before_disclaimer"` with no final submit and no official URL contact; the latest readiness verification returned `caseReadinessStatus: "needs_missing_data"`, `noPreflightCaseReadinessStatus: "needs_official_preflight"`, `reviewedCaseReadinessStatus: "ready_for_human_review"`, `reviewedCaseReadinessOfficialPreflightStatus: "ok"`, `reviewedCaseReadinessCanOpenOfficialSite: true`, `occurredAtReviewStatus: "older_than_review_window"`, `reviewedFieldReviewStatus: "candidate_confirmed_by_user"`, and verified encrypted reporter profile round trip without plaintext fixture identity number or email in the encrypted envelope; the latest prototype readiness gate verification returned `taipeiPrototypeWithoutReadinessStatus: "blocked_by_readiness_report"`, `taipeiPrototypeWithoutPlanFixtureStatus: "blocked_by_plan_fixture_report"`, `taipeiPrototypeWithReadinessStatus: "ready_for_guarded_browser"`, `newTaipeiPrototypeWithTaipeiReadinessStatus: "blocked_by_readiness_report"`, `newTaipeiPrototypeWithoutPlanFixtureStatus: "blocked_by_plan_fixture_report"`, and `newTaipeiPrototypeWithReadinessStatus: "ready_for_guarded_browser"`; the latest guarded handoff verification identified first human stops `stop_before_email_verification` for Taipei and `stop_before_disclaimer` for New Taipei; the latest readiness Markdown verification included `final_submit`, occurred-at freshness warning, confirmed field review, and command hints while omitting fixture identity number and email values; the latest case record Markdown and history summary verification included official case number, returned `caseSummaryOfficialReceiptStatus: "recorded"`, reported `caseSummaryLookupPasswordStored: true`, omitted lookup password value, and preserved pre-submit review plus final-submit human stops; the latest correction tracking verification returned `caseCorrectionStatus: "needs_action"` and `caseCorrectionItemCount: 2`; the latest confirmed-location verification returned one matched frequent-location candidate by GPS distance and OCR text, and UI verification covers case-record import without lookup password exposure, case-history import with lookup-password stored status, case-workflow import, case-readiness import, location candidate confirmation, field suggestion confirmation, OCR location candidate rendering, and confirmed-location candidate confirmation; the latest reporter-profile fixture verification returned `reviewedPacketStatus: "ready_for_human_review"` and `reporterProfileSummaryStatus: "ready"`; the latest metadata verification returned `metadataEmbeddingStatuses: ["sidecar_only", "sidecar_only"]`; the latest UI fixture verification returned `uiFixtureVerification: "ok"` and covers case-record import, case-history import, case-workflow import, case-readiness import, location candidate confirmation, field suggestion confirmation, OCR location candidate rendering, and confirmed-location candidate confirmation; the latest live official preflight reports were written to `cases/taipei-live-preflight.json` and `cases/new-taipei-live-preflight.json` with `status: "ok"`.
